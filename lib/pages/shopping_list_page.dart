...
class _ShoppingListPageState extends State<ShoppingListPage> {
  ...
  Future<void> _deleteList(int index) async {
    ...
    if (confirm == true) {
      final title =
          shoppingLists[index]
              .title; // Find title index and delete the match one in db
                      ...
                      ElevatedButton(
                        ...
                        onPressed: () async {
                          ...
                          if (newTitle.isNotEmpty && newTitle != oldTitle) {
                            // Update the title if it is not empty or same as old title
                            final updatedList = ShoppingList(
                              title: newTitle,
                              ingredients: shoppingLists[index].ingredients,
                            );
                            await DatabaseHelper.instance.deleteShoppingList(
                              oldTitle,
                            );
                          ...
                          if (inputTitle.isNotEmpty) {
                            ...
                            if (alreadyExists) {
                              ScaffoldMessenger.of(context).showSnackBar(
                                const SnackBar(
                                  content: Text(
                                    '⚠️ List with same name already exists!',
                                  ),
                                ),
                              );
                              return;
                            ...
                            try {
                              ...
                              ScaffoldMessenger.of(context).showSnackBar(
                                SnackBar(
                                  content: Text(
                                    '✅ List "$formattedTitle" created successfully',
                                  ),
                                ),
                              );
                            } catch (e) {
                            ...
                            child: ListTile(
                              ...
                              subtitle: Column(
                                ...
                                children: [
                                  ...
                                  if (shoppingLists[i].ingredients.length >
                                      3) // shows '...' if more than 3 ingredients
                                    const Text(
                                      '...',
