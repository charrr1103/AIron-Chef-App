...
class _RecipePageState extends State<RecipePage> {
  ...
  void _decreaseServing() =>
      setState(() => servingSize > 1 ? servingSize-- : servingSize);
  ...
  Future<void> _readRecipe() async {
    ...
    await flutterTts.speak(
      '${recipe!.title}. Ingredients: ${recipe!.ingredients.join(', ')}. Steps: ${recipe!.instructions.join(', ')}.',
    );
  }
  ...
  //compare the ingredient in recipe page with the pantry available ingredients
  Future<void> _compareWithPantry() async {
    final pantryItems = await DatabaseHelper.instance.getAllPantryItems();
    final pantryNames =
        pantryItems.map((item) => item.name.toLowerCase().trim()).toList();
  ...
  Future<void> _toggleSave() async {
    ...
    if (isSaved) {
      await _recipeService.saveRecipe(
        SavedRecipe(id: recipe!.id, title: recipe!.title, image: recipe!.image),
      );
  ...
  Future<void> _downloadAsImage() async {
    try {
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('✅ Recipe saved to: $filePath')));
    } catch (e) {
      ScaffoldMessenger.of(
        context,
  ...
  Future<void> _generateShoppingList() async {
    ...
    final newList = ShoppingList(
      ...
      ingredients:
          missingIngredients
              .map((name) => ShoppingIngredient(name: name))
              .toList(),
    );
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Added missing ingredients to shopping list'),
        ),
      );
    }
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            icon: Icon(isReading ? Icons.stop_circle : Icons.volume_up),
                            ...
                            children: [
                              ...
                              ListView.builder(
                                ...
                                itemBuilder: (context, index) {
                                  ...
                                  final scaledQty = _scaleIngredient(
                                    ingredient,
                                  );
                                  final rest = parts.sublist(1).join(' ');

                                  final isInPantry = pantryNames.any(
                                    (name) => rest.toLowerCase().contains(name),
                                  );
